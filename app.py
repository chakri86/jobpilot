...
/* Layout */
.header{padding:16px 24px;border-bottom:1px solid #1e293b;display:flex;align-items:center;justify-content:space-between;background:linear-gradient(180deg,#0f1525,#0a0e17);flex-wrap:wrap;gap:10px;position:sticky;top:0;z-index:50}
...
/* Buttons */
.btn{padding:10px 20px;border-radius:8px;border:none;font-size:13px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;display:inline-flex;align-items:center;gap:6px;transition:all 0.15s}
...
.avatar{width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#3b82f6,#a78bfa);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;color:#fff;flex-shrink:0}
...
/* Dropzone */
.dz{border:2px dashed #1e293b;border-radius:10px;padding:30px;text-align:center;cursor:pointer;transition:all 0.2s;background:#0a0e17;margin-bottom:12px}
.dz:hover,.dz.over{border-color:#3b82f6;background:rgba(59,130,246,0.03)}
...
<form method="POST" action="{{ '/p/' ~ p.id ~ '/save' if p else '/create' }}">
  <div class="card"><div class="card-title">👤 Personal Info</div>
    <div class="form-row"><div><label>Name *</label><input name="name" id="fn" value="{{ p.name if p else '' }}" required></div><div><label>Email</label><input name="email" id="fe" value="{{ p.email if p else '' }}" type="email"></div></div>
  ...
  <div class="card"><div class="card-title">🎯 Job Preferences</div>
    ...
    <div class="form-row"><div><label>Skills</label><input name="skills" id="fs" value="{{ p.skills if p else '' }}" placeholder="Agile, Jira, Risk Analysis"></div><div><label>Min Salary</label><input name="min_salary" value="{{ p.min_salary if p else '' }}"></div></div>
    <div class="form-row"><div><label>Job Type</label><select name="job_type"><option value="fulltime" {{ 'selected' if p and p.job_type=='fulltime' }}>Full Time</option><option value="parttime" {{ 'selected' if p and p.job_type=='parttime' }}>Part Time</option><option value="contract" {{ 'selected' if p and p.job_type=='contract' }}>Contract</option></select></div><div><label>Min Score to Notify (0-100)</label><input name="min_score" type="number" value="{{ p.min_score if p else 75 }}"></div></div>
  </div>

  <div class="card"><div class="card-title">📄 Resume</div>
    <div class="dz" id="dz" ondragover="event.preventDefault();this.classList.add('over')" ondragleave="this.classList.remove('over')" ondrop="event.preventDefault();this.classList.remove('over');doUpload(event.dataTransfer.files[0])">
    ...
    <div class="form-row full"><div><label>Resume Text <span style="font-weight:400;color:#64748b">(auto-filled from upload or paste manually)</span></label><textarea name="resume_text" id="fr" rows="12">{{ p.resume_text if p else '' }}</textarea></div></div>
  </div>

  <div class="card"><div class="card-title">🔑 API Key</div>
    <div class="form-row full"><div><label>Anthropic API Key</label><input name="anthropic_api_key" type="password" value="{{ p.anthropic_api_key if p else '' }}" placeholder="sk-ant-..."><div class="help">Get at <a href="https://console.anthropic.com/" target="_blank">console.anthropic.com</a></div></div></div>
  </div>
  ...
  <div class="card"><div class="card-title">📧 Email Alerts</div>
    ...
    <div class="form-row"><div><label>Gmail</label><input name="email_sender" value="{{ p.email_sender if p else '' }}"></div><div><label>App Password</label><input name="email_password" type="password" value="{{ p.email_password if p else '' }}"><div class="help"><a href="https://myaccount.google.com/apppasswords" target="_blank">Get App Password</a></div></div></div>
  ...
  <div class="card"><div class="card-title">💬 Telegram</div>
    ...
    <div class="form-row"><div><label>Bot Token</label><input name="telegram_bot_token" value="{{ p.telegram_bot_token if p else '' }}"><div class="help">From <a href="https://t.me/BotFather" target="_blank">@BotFather</a></div></div><div><label>Chat ID</label><input name="telegram_chat_id" value="{{ p.telegram_chat_id if p else '' }}"><div class="help">From <a href="https://t.me/userinfobot" target="_blank">@userinfobot</a></div></div></div>
  </div>
...
<script>
document.getElementById('dz').onclick=function(){document.getElementById('rf').click()};
function doUpload(f){if(!f)return;var s=document.getElementById('us');s.style.display='block';s.style.color='#3b82f6';s.textContent='⏳ Parsing '+f.name+'...';var d=new FormData();d.append('file',f);fetch('/upload',{method:'POST',body:d}).then(r=>r.json()).then(j=>{if(j.ok){document.getElementById('fr').value=j.text;s.style.color='#22c55e';s.textContent='✅ '+j.text.length+' chars extracted from '+f.name;if(j.name&&!document.getElementById('fn').value)document.getElementById('fn').value=j.name;if(j.email&&!document.getElementById('fe').value)document.getElementById('fe').value=j.email;if(j.phone&&!document.getElementById('fp').value)document.getElementById('fp').value=j.phone}else{s.style.color='#ef4444';s.textContent='❌ '+j.err}}).catch(e=>{s.style.color='#ef4444';s.textContent='❌ '+e})}
</script>
{% endif %}
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:8px">
  ...
  <div style="display:flex;gap:6px"><form method="POST" action="/p/{{ p.id }}/toggle"><button class="btn {{ 'btn-d' if p.is_active else 'btn-g' }} btn-sm">{{ '⏸ Pause' if p.is_active else '▶️ Start' }}</button></form><a href="/p/{{ p.id }}/edit"><button class="btn btn-s btn-sm">✏️</button></a></div>
</div>
<div class="sgrid">
...
{% for j in topj[:10] %}<div class="jrow"><div class="sc {{'sc-h' if j.match_score>=80 else 'sc-m' if j.match_score>=60 else 'sc-l'}}">{{j.match_score}}</div><div style="flex:1"><div style="font-size:14px;font-weight:700">{{j.title}}</div><div style="font-size:12px;color:#94a3b8">{{j.company}} · {{j.location}}</div></div>{% if j.apply_url %}<a href="{{j.apply_url}}" target="_blank"><button class="btn btn-p btn-sm">Apply →</button></a>{% endif %}</div>{% endfor %}</div>{% endif %}
{% endif %}
...
<div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:700">{{j.title}}{% if j.applied %} <span class="bg bg-g">Applied</span>{% endif %}</div><div style="font-size:12px;color:#94a3b8">{{j.company}} · {{j.location}}</div></div>
<div style="display:flex;gap:4px">{% if not j.applied %}<a href="/j/{{j.id}}/apply"><button class="btn btn-g btn-sm">✅</button></a>{% endif %}{% if j.apply_url %}<a href="{{j.apply_url}}" target="_blank"><button class="btn btn-p btn-sm">Open</button></a>{% endif %}</div></div>{% endfor %}</div>{% endif %}
{% endif %}
...
<form method="POST" action="/j/{{j.id}}/status"><select name="st" onchange="this.form.submit()" style="width:auto;font-size:11px;padding:4px 8px">{% for s in ['applied','interview','offer','rejected','ghosted'] %}<option value="{{s}}" {{'selected' if j.status==s}}>{{s}}</option>{% endfor %}</select></form></div>{% endfor %}</div>{% endif %}
{% endif %}
</div></body></html>"""
...
def create():
    ...
    cur = c.execute("INSERT INTO profiles (name,email,phone,location,resume_text,skills,target_titles,job_type,min_salary,min_score,check_interval,anthropic_api_key,email_enabled,email_sender,email_password,email_receiver,telegram_enabled,telegram_bot_token,telegram_chat_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (d.get("name"),d.get("email"),d.get("phone"),d.get("location"),d.get("resume_text"),d.get("skills"),d.get("target_titles"),d.get("job_type","fulltime"),d.get("min_salary"),int(d.get("min_score",75)),int(d.get("check_interval",15)),d.get("anthropic_api_key"),1 if "email_enabled" in d else 0,d.get("email_sender"),d.get("email_password"),d.get("email_receiver"),1 if "telegram_enabled" in d else 0,d.get("telegram_bot_token"),d.get("telegram_chat_id")))
    pid = cur.lastrowid
